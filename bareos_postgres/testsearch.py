...
#         self._set_config(parser, args, kwargs) # NEVER REMOVE
    ...
    def main(self):
        ...
        _sql = """SELECT file.jobid, job.starttime, path.path, file.name   
                  ...
                  """
        for i in conn.ENGINE.execute(_sql):
            _jobid = i[0]
            _start = i[1].strftime('%Y-%m-%d')
            _fullpath = os.path.join(i[2], i[3]) 
            ...
            #===================================================================
            # pathids.append(i[0])
            # pathnames.append(i[1])
            # 
            # try: print("Already found '{}'".format(paths[i[1]]))
            # except KeyError:
            #     print("A", end="")                 
            #     paths[i[1]] = i[0]
            #===================================================================
...
#             _pathid = i[0]
#             _path = i[1]
#             print("pathid:{}, path:{}".format(_pathid, _path))
#             a = input(...)
#             continue
#             
#             _sql = "SELECT jobid, name FROM file WHERE pathid = {}".format(_pathid)
#             for j in conn.ENGINE.execute(_sql):
#                 print(j)
# #                 print("jobid:{}, name:{}".format(_pathid, _path))
#===============================================================================
