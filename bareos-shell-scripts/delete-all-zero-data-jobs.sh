...
# T     Terminated normally
# W     Terminated with warnings
while read spacer JOBID spacer NAME spacer CLIENT spacer STARTDATE STARTTIME spacer TYPE spacer LEVEL spacer JOBFILES spacer JOBBYTES spacer STATUS spacer
do
    if [ ${JOBFILES} == "0" ] && [ ${JOBBYTES} == "0" ] && [ ${LEVEL} != "F" ]; then

        if echo $STATUS | grep -q "[W|T|A]"; then
